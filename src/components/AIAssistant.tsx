...
import { MessageSquare, X, Send, Bot, User, Mail, MessageSquare as MessageIcon, Phone, Mic, Volume2 } from 'lucide-react';
...
interface Message {
  role: 'user' | 'assistant';
  content: string;
  audio?: string; // Base64 audio for TTS
}
...
export default function AIAssistant() {
  ...
  const handleSend = async () => {
    ...
    try {
      // Use VITE_ prefix for Vercel/Vite deployments, fallback to process.env for AI Studio preview
      ...
      // 1. Generate Text Response with Thinking Mode (if complex) or Standard
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          tools: tools,
          thinkingConfig: { thinkingLevel: ThinkingLevel.HIGH } // Enable thinking mode for complex queries
        }
      });
      ...
      if (functionCalls) {
        ...
        aiText = response.text || "I'm sorry, I'm having trouble connecting right now. Please try again later or contact Naman directly.";
      }
      ...
      const errorMessage = error.message?.includes("Gemini API Key is missing") 
        ? "I'm not configured yet. Please add VITE_GEMINI_API_KEY to your Vercel environment variables."
        : "I encountered an error. Please contact Naman directly at namanalahariya@gmail.com.";
            ...
            >
              ...
              <div className="p-4 border-b border-theme bg-accent/5 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button 
                    onClick={() => { setIsLiveOpen(true); setIsOpen(false); }}
